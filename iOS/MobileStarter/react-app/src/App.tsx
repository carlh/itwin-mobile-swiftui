...
const appReduxStore: Store<RootState> = createStore(
  rootReducer,
...
function App() {
  ...
  const pushActiveInfo = React.useCallback(
    (screen: ActiveScreen, cleanup?: () => void) => {
  ...
  const handleOpen = React.useCallback(
    async (filename: string, newIModelPromise: Promise<IModelConnection>) => {
      try {
  ...
  React.useEffect(() => {
    if (initialized) {
      ...
      //      Just need to figure out how to handle iModel downloading and setting the path correctly.

      return Messenger.onQuery('openModel').setHandler(async (modelPath: string) => {
        if (activeScreen === ActiveScreen.Model) {
          // CHIN: Maybe we can take a viewportID or a param indicating the need to open in a new viewport to support multiple windows/viewports/connections
