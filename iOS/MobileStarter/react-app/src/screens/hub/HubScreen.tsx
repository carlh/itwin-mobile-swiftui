...
export function HubScreen(props: HubScreenProps) {
  ...
  const selectProject = React.useCallback(
    async (project: ProjectInfo) => {
      ...
      const hubIModels = (await hubClient.iModels.get(requestContext, project.wsgId)).sort((a, b) =>
  ...
  const openIModel = React.useCallback(
    (briefcase: LocalBriefcaseProps) => {
  ...
  const downloadIModel = React.useCallback(
    async (iModel: HubIModel) => {
      setButtonTitles([]);
  ...
  const handleSelectIModel = React.useCallback(
    async (index) => {
    ...
    if (hubStep === HubStep.SelectIModel || hubStep === HubStep.DownloadIModel) {
      ...
      return (
        <div className="imodel-row" key={index}>
          <Button onClick={() => handleSelectIModel(index)} title={getTitle(title, briefcase)}>
            {deleteButton}
          </Button>
        </div>
      );
  ...
  if (haveCachedBriefcase || project) {
    ...
    moreButton = <ActionSheetButton actions={actions} />;
  }
    ...
    <Screen>
      <div className="hub-screen">
        <div className="title">
          ...
          <div className="more-parent">{moreButton}</div>
        </div>
        <div className="list">
